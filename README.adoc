...
[NOTE]
====
The initial release was done as version `0.x` because we are still considering changing the library name as well maven coordinates and namespaces. However, aside from simple changes to the import statements, we expect `1.x` to be fully compatible.
====
