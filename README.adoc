...
The serialization wrappers are very light and serialize the contained data into the `MAT File Format` directly.

```Java
// Add EJML Matrix
MatFile mat = Mat5.newMatFile();
mat.addArray("ejmlMatrix", Mat5Ejml.asArray(new DMatrixRMaj(10, 10)))
```

Existing Matrices in `MAT Files` can be converted to EJML types via `output = Mat5Ejml.convert(matrix, output)`.

```Java
// Convert Matrix to EJML Type
MatFile mat = Mat5.newMatFile();
DMatrixRMaj ejmlMatrix = Mat5Ejml.convert(mat.getArray("ejmlMatrix"), new DMatrixRMaj(0, 0));
```
